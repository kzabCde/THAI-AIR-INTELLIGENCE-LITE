...
export function ProvinceDetailClient({ slug }: { slug: string }) {
  ...
  if (isLoading) return <p>กำลังโหลดข้อมูลจังหวัด...</p>;
  if (error) return <p className="text-rose-600">โหลดข้อมูลไม่สำเร็จ: {error}</p>;
  if (!province) {
    return (
      <Card>
        <h1 className="text-xl font-bold">ยังไม่มีข้อมูลของ {provinceMeta?.province_name_th ?? slug}</h1>
        <p className="mt-2 text-sm text-slate-500">ระบบจะลองโหลดใหม่อัตโนมัติทุก 60 วินาที</p>
      </Card>
    );
  }
