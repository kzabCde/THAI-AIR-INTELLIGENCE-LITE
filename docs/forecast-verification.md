...
สถานะ: ใช้ migration และประเมินย้อนหลังใหม่บน Production แล้ววันที่ 2026-09-13; PR #77 ถูก merge แล้ว; ขั้นตอนนี้ไม่ได้สั่ง deploy หรือยืนยัน deployment ของ API/UI
...
## ตรวจรอบอัตโนมัติและแก้ CI 2026-09-14

- เวลา 01:30 น. ไทย evaluator เพิ่มผลวันที่ 13 ก.ย. 200 รายการ ครบ 20 จังหวัด ทุกผลมี 24 ชั่วโมง; v2 final รวม 7,275 รายการ ณ เวลาตรวจ. Drift มีรายการใหม่ในรอบเดียวกันด้วย.
- การสร้างคำทำนายใหม่สองรอบล่าสุดยังจบด้วย Gateway Timeout (504). ยืนยันได้ว่า evaluation และ drift ทำงานแล้ว แต่ error เดิมไม่ได้บันทึกชื่อขั้นตอนที่ล้ม จึงยังระบุ RPC ต้นเหตุไม่ได้ และยังไม่ยืนยัน pipeline สำเร็จทั้งรอบ.
- อัปเดต Next.js/eslint-config-next เป็น 15.5.25, PostCSS/override เป็น 8.5.28 และ sharp override เป็น 0.35.4 ตาม dependency audit. รายละเอียด: [Next.js advisory](https://github.com/advisories/GHSA-2xp9-vwfh-vxw4), [PostCSS advisory](https://github.com/advisories/GHSA-fxqj-rqcc-2cmp), [sharp advisory](https://github.com/advisories/GHSA-rgj7-g3m4-5g8c).
- แก้ test สิ้นวันให้ใช้วันที่ Asia/Bangkok ตรงกับ evaluator แทน current_date ที่ขึ้นกับ timezone ของ session.
