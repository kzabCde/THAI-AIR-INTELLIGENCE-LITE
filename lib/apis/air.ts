...
async function fetchOpenMeteoAir(lat: number, lon: number): Promise<AirReading | null> {
  try {
    ...
    const pm25Values: number[] = json?.hourly?.pm2_5 ?? [];
    const pm10Values: number[] = json?.hourly?.pm10 ?? [];
...
async function fetchOpenAQ(lat: number, lon: number): Promise<AirReading | null> {
  try {
    ...
    const pm25Sensor = sensors.find((s: any) => String(s?.parameter?.name).toLowerCase().includes("pm25"));
    const pm10Sensor = sensors.find((s: any) => String(s?.parameter?.name).toLowerCase().includes("pm10"));
...
async function fetchAir4Thai(lat: number, lon: number): Promise<AirReading | null> {
  // Public endpoint availability changes often; this optional connector is soft-fail.
  try {
    ...
    const pm25 = Number(entry?.PM25?.value ?? entry?.pm25 ?? 0);
    if (!pm25) return null;
    const pm10 = Number(entry?.PM10?.value ?? entry?.pm10 ?? pm25 * 1.3);
    ...
    return {
      ...
      source: "air4thai",
      station: entry?.nameEN ?? entry?.nameTH ?? "Air4Thai station",
      fetchedAt: new Date().toISOString(),
    };
  } catch {
    return null;
